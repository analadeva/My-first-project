...
@endif
        ...
        <div class="d-flex justify-content-between">
                    <p style="font-size:1.5rem;"><a href="{{route('product')}}" class="mr-3 text-dark"><i class="fas fa-arrow-left"></i></a><b>Уреди Бренд</b></p>
                    <div class="form-group">
    <select class="form-control" id="exampleFormControlSelect1" name="status"> 
        <option selected disabled>Статус</option>
        <option value="1" {{ old('status', $brand->status) == 1 ? 'selected' : '' }}>Активен</option>
    <option value="0" {{ old('status', $brand->status) == 0 ? 'selected' : '' }}>Неактивен</option>
    </select>
</div>
        </div>
    ...
    <input type="text" class="form-control" id="exampleInputEmail1" name="name" aria-describedby="emailHelp" value="{{ old('name', $brand->name) }}">
        </div>

        <div class="form-group">
    <label for="exampleFormControlTextarea1">Опис</label>
    <textarea class="form-control" id="exampleFormControlTextarea1" name="disc" rows="3">{{ old('name', $brand->disc) }}</textarea>
  </div>
